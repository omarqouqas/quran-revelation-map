...
| Feature | Description |
|---------|-------------|
| **Global Search** | Unified search modal (Cmd/Ctrl+K) that searches across surahs, events, and sacred sites. Features categorized results with scoring algorithm (exact match > starts with > contains), keyboard navigation (arrow keys, Enter to select, Esc to close), debounced input, and quick navigation to results. Clicking results opens relevant panels (SurahDetailPanel, EventDetailModal, SacredSiteModal) and navigates timeline. |
